...
char *gSys_settings[] = {"WkTemp ", "StbTemp", "SlpTime", "Power  ", "TempStp", "OffVolt", "Temp   ", "Hand   ", "\0", "\0"};
...
static s32 gSet_table[9][3] = {{4000, 1000, 100}, //工作温度
    {4000, 1000, 100}, //休眠温度
...
s16 Temp_conversion(u8 flag , s16 tmp)
{
    if(flag == 0)
    {
        return (320 + tmp * 9 / 5); //C -> F
    }
    else
    {
        return (5 * tmp - 1600) / 9; //F -> C
    }
}
/*******************************************************************************
函数名: APP_Init
