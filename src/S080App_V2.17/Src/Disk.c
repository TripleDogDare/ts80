...
//const char *gKey_words[] = {"StbTemp","WkTemp","SlpTime","Rated_P","TempStp","OffVolt","Temp","Hand","ZeroP_Ad"};
...
const u8 *gDef_set[]   = {  CONS(STBTEMP_STR,  STBTEMP_VAL)  , CONS(WKTEMP_STR, WKTEMP_VAL) ,
                            ...
                            CONS(ZEROCALI_STR, ZEROCALI_VAL)
                         };
...
static u8 isValidConfg(const u8 ini[][ROW_CONT], DEVICE_INFO_SYS *config)
{
    ...
    if(valueStr = strchr((char *)ini[6], '='))
    {
        ...
        if(value != TEMP_UNIT_C && value != TEMP_UNIT_F)
            return 0;
        T_Unit = value;
    }
    else
    ...
    for(u16 i = 0; i < 2; i++)
    {
        ...
        {
            if(value < 212 || value > 752)
                return 0;
        }
        if(i == 0)
            config->t_standby = value * 10;
        else
            config->t_work = value * 10;
    }
    //休眠时间
